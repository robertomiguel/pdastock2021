...
export interface IFiscalCategoryType {
    _id: string
    name: string
    shortname: string
}
...
export interface IFiscalCategoryStore {
    list: Partial<IFiscalCategoryType>[]
    getList: () => Promise<boolean>
    getById: (id: string) => Promise<boolean>
    isLoading: boolean
    item: IFiscalCategoryType | {} | any
    createUpdate: (data: Partial<IFiscalCategoryType>) => Promise<boolean>
...
const FiscalCategoryStore = () =>
    observable<IFiscalCategoryStore>({
        ...
        async getList() {
            this.isLoading = true
            const list: IFiscalCategoryType[] = await connection.fiscalCategory(
        ...
        async getById(id) {
            const data: IFiscalCategoryType[] = await connection.fiscalCategory(
                {
                    filter: { _id: id },
                    options: {
                        limit: 1,
